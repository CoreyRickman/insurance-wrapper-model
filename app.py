...
from engine.ppva import run_ppva_accum
...
from engine.formatting import fmt_money

st.set_page_config(page_title="Taxable vs PPVA vs PPLI (Planning)", layout="wide")
st.title("Taxable vs PPVA vs PPLI — Planning Model")
...
with st.sidebar:
    ...
    st.subheader("Taxable return composition")
...
with tabs[0]:
    ...
    if st.button("Run model", type="primary"):
        ...
        if do_ppva:
            ...
            summ = summarize_path("PPVA", df_ppva, "value_end")
        ...
        if do_ppli:
            ...
            summ = summarize_path("PPLI", df_ppli, "cash_value_end", cashflow_col="loan_gross", tax_col="loan_tax")
    ...
    if not st.session_state["summary"].empty:
        ...
        chart_col = "ending_value_after_tax_exit" if "ending_value_after_tax_exit" in summary.columns else "ending_value"

        st.bar_chart(summary.set_index("scenario")[chart_col])
...
with tabs[1]:
    ...
    if df is None:
        ...
        st.download_button("Download taxable CSV", df.to_csv(index=False).encode("utf-8"), "taxable.csv")

with tabs[2]:

    st.subheader("PPVA Schedule")

    df = st.session_state["results"].get("PPVA")
    ...
    if df is None:
        ...
        for c in ["value_start", "value_end", "basis"]:
            if c in df_show.columns:
                df_show[c] = df_show[c].map(lambda v: fmt_money(float(v)))

        st.dataframe(df_show, use_container_width=True)

        st.download_button("Download PPVA CSV", df.to_csv(index=False).encode("utf-8"), "ppva.csv")
...
with tabs[3]:
    ...
    if df is None:
        ...
        for c in ["cash_value_start", "cash_value_end", "loan_balance"]:
            if c in df_show.columns:
                df_show[c] = df_show[c].map(lambda v: fmt_money(float(v)))

        st.dataframe(df_show, use_container_width=True)

        st.download_button("Download PPLI CSV", df.to_csv(index=False).encode("utf-8"), "ppli.csv")
